 ...
 */
export function Popover({ icon, title, active, align = 'up', children }: Props): JSX.Element {
  ...
  return (
    <div className="pop" ref={root}>
      <button
        className={`togglebtn ${active ? 'togglebtn--on' : ''}`}
        title={title}
        aria-expanded={open}
      ...
      {open && (
        <div className={`pop__panel pop__panel--${align}`} role="dialog" aria-label={title}>
